...
function ReminderItem({ reminder, onTaken, onSkipped, onSnooze }) {
  const { theme } = useTheme()
  const isLight = theme === 'light'
    ...
    <div className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${
      ...
      {isPending && (
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            aria-label={`Snooze ${reminder.medicine_name} 10 minutes`}
            title="Snooze 10 minutes"
            onClick={() => onSnooze(reminder.id)}
            className={`p-1.5 rounded-xl text-xs font-medium transition-all ${
              isLight ? 'bg-navy-100 text-navy-400 hover:bg-cyan-100 hover:text-cyan-700' : 'bg-white/[0.06] text-white/40 hover:bg-cyan-500/20 hover:text-cyan-400'
            }`}
          >
          ...
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
            isSnoozed
              ...
              : isDue
                ...
                : (isLight ? 'bg-cyan-100 text-cyan-700' : 'bg-cyan-500/20 text-cyan-400')
          }`}>{isSnoozed ? 'Snoozed' : isDue ? 'Due' : 'Upcoming'}</span>
        </div>
      )}
...
export default function Dashboard() {
  ...
  const handleSnooze = async (id) => {
    try {
      ...
      setError('Could not snooze reminder. Please try again.')
    }
  }
              ...
              <motion.div variants={itemAnim} className={`${isLight ? 'bg-white rounded-3xl shadow-sm border border-navy-100' : 'glass-card'} p-5 md:p-6`}>
                ...
                <div className="space-y-2">
                  {reminders.length === 0 ? (
                    ...
                    reminders.map(rem => (
                      <ReminderItem key={rem.id} reminder={rem} onTaken={handleTaken} onSkipped={handleSkipped} onSnooze={handleSnooze} />
                    ))
                  )}
                </div>
              </motion.div>
