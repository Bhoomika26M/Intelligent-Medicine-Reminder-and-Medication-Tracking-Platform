...
const EditMedicine = () => {
  ...
  // Auto-updates times if boxes are adjusted (same helpful feature as add screen)
  const [userInteracted, setUserInteracted] = useState(false);
  useEffect(() => {
    if (!userInteracted) {
      // Avoid overwrite on initial load
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.detail || "Failed to update medicine. Check inputs and try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-800">Edit Medicine Treatment</h1>
        <p className="text-sm text-slate-500 mt-1">Modify dose schedules, instruction notes, and inventory status.</p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full rounded-xl border py-2.5 px-3.5 text-sm outline-none transition-all ${
                errors.remaining_stock ? 'border-rose-300 focus:border-rose-400' : 'border-slate-200 focus:border-blue-500'
              }`}
            />
            {errors.remaining_stock && <span className="text-xs text-rose-500 mt-1 block">{errors.remaining_stock.message}</span>}
          </div>
        </div>
